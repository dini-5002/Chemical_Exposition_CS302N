...
enum Symbol {
    ...
    Sb,Te,I,Xe,Cs,Ba,Hf=72,Ta,W,Re,
    Os,Ir,Pt,Au,Hg,Tl,Pb,Bi,Po,At,Rn,Fr,Ra
};

struct CPKColor { float r, g, b; };
...
CPKColor getCPK(int sym) {
    ...
    if (it != cpkColors.end()) return it->second;
    float r=0,g=0,b=0;
    int index = sym % 6, row = sym / 6;
    if (index==0||index==3||index==4) r = 1.0f/(row+1);
    if (index==1||index==3||index==5) g = 1.0f/(row+1);
    if (index==2||index==4||index==5) b = 1.0f/(row+1);
    if (r+g+b < 0.1f) { r=0.7f; g=0.7f; b=0.7f; }
    return {r,g,b};
}

map<int,float> vdwRadius = {
    {H,1.2f},{C,1.7f},{N,1.55f},{O,1.52f},
    {S,1.8f},{P,1.8f},{Cl,1.75f},{Br,1.85f},{I,1.98f}
};
...
struct PeriodicTable {
    ...
    bool AtomsCanBond(Atom a1, Atom a2) {
        ...
        auto i1 = find_if(table.cbegin(),table.cend(),[&a1](AtomDetail x){return x.symbol==a1.symbol;});
        auto i2 = find_if(table.cbegin(),table.cend(),[&a2](AtomDetail x){return x.symbol==a2.symbol;});
        if (i1==table.cend()||i2==table.cend()) return false;
        ...
        while (getline(stream, line)) {
            ...
            if (sscanf(line.c_str(),"%s %lf %lf %lf",atomName,&x,&y,&z)==4) {
                auto it = find_if(chem.table.cbegin(),chem.table.cend(),
                    [&atomName](AtomDetail x){ return x.name==atomName; });
                if (it==chem.table.cend()) continue;
                Atom a((*it).symbol); a.x=x; a.y=y; a.z=z;
...
void stringPrint(const string& s,int x,int y,float r,float g,float b) {
    ...
    glRasterPos2i(x,y);
    for (char c : s) {
        if (c=='\n') { qp+=20; glRasterPos2i(x,y-qp); }
        else { glColor3f(r,g,b); glutBitmapCharacter(font,c); }
    }
}
void stringPrinter(const string& s,int x,int y,float r,float g,float b) {
    if (textMode) stringPrint(s,x,y,r,g,b);
}
              ...
              bool isSelected)
{
    ...
    if (isSelected) { col.r=1.0f; col.g=1.0f; col.b=0.0f; }
...
void drawOrbitingElectrons(float cx, float cy, float cz, float atomR, int symInt) {
    if (!electronOrbitMode || atomR <= 1e-5f) return;
...
void drawReaction() {
    ...
    for (int i=0, k=-1; i<3; i++, k++) {
        ...
        if (pk==-1 || pk>0) {
            ...
            if (atomMode) {
                for (auto& atom : molecule[i].atoms) {
                    ...
                    string symName="?";
                    if (it1!=chem.table.cend()) symName=(*it1).name;
            ...
            if (i==0) {
                ...
                GLUquadricObj* q=gluNewQuadric();
                gluQuadricNormals(q,GLU_SMOOTH);
                renderCylinder(4,0,0,5,0,0,0.12f,20,q); gluDeleteQuadric(q);
                q=gluNewQuadric(); gluQuadricNormals(q,GLU_SMOOTH);
                renderCylinder(4.5f,-0.5f,0,4.5f,0.5f,0,0.12f,20,q); gluDeleteQuadric(q);
                glEnable(GL_LIGHTING);
            } else if (i==1) {
                glDisable(GL_LIGHTING);
                ...
                GLUquadricObj* q=gluNewQuadric();
                gluQuadricNormals(q,GLU_SMOOTH);
                renderCylinder(-4.5f,0.25f,0,-5.5f,0.25f,0,0.12f,20,q); gluDeleteQuadric(q);
                q=gluNewQuadric(); gluQuadricNormals(q,GLU_SMOOTH);
                renderCylinder(-4.5f,-0.25f,0,-5.5f,-0.25f,0,0.12f,20,q); gluDeleteQuadric(q);
                glEnable(GL_LIGHTING);
            }
        ...
        if (pk!=-1) pk--;
        if (pk==1 && k==1) { presentation=1; moleI++; glutPostRedisplay(); }
    }
}
...
void display() {
    switch(value) {
        ...
        case 21: atomMode=true;        value=0; createMenu(); break;
        case 30: bondMode=false;       value=0; createMenu(); break;
        case 31: bondMode=true;        value=0; createMenu(); break;
        case 40: atomSize+=0.1f;       value=0; break;
        case 41: atomSize-=0.1f;       value=0; break;
        case 50: presentation=-1; moleI=0; value=0; createMenu(); break;
        case 51: presentation=0;  moleI=0; value=0; createMenu(); break;
        case 60: textMode=0;           value=0; createMenu(); break;
        case 61: textMode=1;           value=0; createMenu(); break;
        case 70: floorMode=0;          value=0; createMenu(); break;
        case 71: floorMode=1;          value=0; createMenu(); break;
        case 80: shellMode=false;      value=0; createMenu(); break;
        case 81: shellMode=true;       value=0; createMenu(); break;
        case 82: fogEnabled=false; setupFog(); value=0; createMenu(); break;
        case 83: fogEnabled=true;  setupFog(); value=0; createMenu(); break;
        case 84: autoSpinActive=false; value=0; createMenu(); break;
        case 85: autoSpinActive=true;  value=0; createMenu(); break;
        case 86: labelMode=false;      value=0; createMenu(); break;
        case 87: labelMode=true;       value=0; createMenu(); break;
        case 88: shadowMode=false;     value=0; createMenu(); break;
        case 89: shadowMode=true;      value=0; createMenu(); break;
        case 90: envMapMode=false;     value=0; createMenu(); break;
        case 91: envMapMode=true;      value=0; createMenu(); break;
        case 92: atomSelfSpinActive=false; value=0; createMenu(); break;
        case 93: atomSelfSpinActive=true;  value=0; createMenu(); break;
        case 96: perspectiveProj=false;    value=0; createMenu();
                 glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT)); break;
        case 97: perspectiveProj=true;     value=0; createMenu();
                 glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT)); break;
        case 100: electronOrbitMode=false; value=0; createMenu(); break;
        case 101: electronOrbitMode=true;  value=0; createMenu(); break;
    }
    ...
    string status="";
    if (autoSpinActive) status+="[SCENE-SPIN] ";
    if (atomSelfSpinActive) status+="[ATOM-SPIN] ";
    status += perspectiveProj ? "[PERSP] " : "[ORTHO] ";
    if (electronOrbitMode) status += "[e- ORBIT] ";
    if (fogEnabled)     status+="[FOG] ";
...
void mouseFunc(int button, int state, int x, int y) {
    ...
    if (state==GLUT_DOWN && start==0) {
        presentation=-1; start=-1; createMenu();
    ...
    if (button==GLUT_LEFT_BUTTON) {
        ...
        if (state==GLUT_DOWN) autoSpinActive=false;
    }
    if (button==GLUT_MIDDLE_BUTTON) middleDown=(state==GLUT_DOWN);

    if (button==3) {
        if (perspectiveProj) { camDist-=0.8f; if(camDist<1.0f)camDist=1.0f; }
        else { orthoHalfHeight *= 0.92f; if (orthoHalfHeight < 3.0f) orthoHalfHeight = 3.0f;
               glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT)); }
    }
    if (button==4) {
        if (perspectiveProj) { camDist+=0.8f; }
        else { orthoHalfHeight *= 1.08f; if (orthoHalfHeight > 80.0f) orthoHalfHeight = 80.0f;
               glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT)); }
    }
    ...
    if (button==GLUT_LEFT_BUTTON && state==GLUT_DOWN && start!=0) {
        ...
        if (presentation==-2) { presentation=-1; moleI=0; createMenu(); }
        if (presentation!=-1 && index==0 && presentation!=-2) presentation+=1;
    }
...
void mouseMotion(int x, int y) {
    ...
    if (leftDown)   { camRotY+=dx*0.5f; camRotX+=dy*0.5f; }
    if (middleDown) { panX+=dx*0.01f;   panY-=dy*0.01f;   }
    lastMouseX=x; lastMouseY=y;
    glutPostRedisplay();
}
...
void timerCB(int) {
    if (autoSpinActive) {
        ...
        if (autoSpinAngle>=360.0f) autoSpinAngle-=360.0f;
        glutPostRedisplay();
    }
    if (atomSelfSpinActive) {
        atomSpinAngle += atomSpinSpeed;
        if (atomSpinAngle >= 360.0f) atomSpinAngle -= 360.0f;
        glutPostRedisplay();
    }
...
void keyboardFunc(unsigned char key, int x, int y) {
    switch (key) {
        case 'w': panY+=0.5f; break;
        case 's': panY-=0.5f; break;
        case 'a': panX-=0.5f; break;
        case 'd': panX+=0.5f; break;
        ...
        case 'e': shellMode=!shellMode;     createMenu(); break;
        case 'v': envMapMode=!envMapMode;   createMenu(); break;
        case 'l': labelMode=!labelMode;     createMenu(); break;
        case 'h': shadowMode=!shadowMode;   createMenu(); break;
        case 'b': case 'B':
            electronOrbitMode = !electronOrbitMode;
            createMenu();
            break;
        case 'o': case 'O':
            atomSelfSpinActive=!atomSelfSpinActive; createMenu(); break;
        case 'g': case 'G':
        ...
        case '[':
            if (!perspectiveProj) {
                ...
                if (orthoHalfHeight > 80.0f) orthoHalfHeight = 80.0f;
                glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT));
            }
            break;
        case ']':
            if (!perspectiveProj) {
                ...
                if (orthoHalfHeight < 3.0f) orthoHalfHeight = 3.0f;
                glutReshapeWindow(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT));
            }
            break;
...
void specialKeyboard(int key, int, int) {
    switch(key) {
        case GLUT_KEY_UP:    camDist-=0.8f; if(camDist<1.0f)camDist=1.0f; break;
        case GLUT_KEY_DOWN:  camDist+=0.8f; break;
